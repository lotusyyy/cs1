...
void print_statistics(struct world_t *world) {
  ...
  print_map_statistics(number_of_dirt_tiles, number_of_gem_tiles,
      number_of_boulder_tiles, completion_percentage, maximum_points_remaining);
}
...
int boulder_move(struct world_t *world) {
  ...
  for (int row = 0; row < ROWS; row++) {
    for (int col = 0; col < COLS; col++) {
      if (world->board[row][col].entity == BOULDER && row == world->player_row
          && col == world->player_col) {
        return TRUE;
      }
    }
  }
  ...
  for (int row = 0; row < ROWS; row++) {
    for (int col = 0; col < COLS; col++) {
      double distance = sqrt(
          pow(row - world->player_row, 2) + pow(col - world->player_col, 2));
      if (world->lavas[row][col]) {
